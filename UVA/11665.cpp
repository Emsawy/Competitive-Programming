...
V<point> ConvexHull(V<point> points){
	...
	*/
	...
	for (int i = 0; i < points.size(); i++){
		while (ch.size() > 1 && cross(ch[sz(ch) - 2] - ch[sz(ch) - 1], points[i] - ch[sz(ch) - 1]) < 0)
...
pair< V <point>, V < point> > polygonCut(V<point> & p, point A, point B){
	...
	for (int i = 0; i < p.size(); i++){
		...
		point p;
		if (intersectionSegments(A, B, cur, nxt, p)){
			right.push_back(p);
			left.push_back(p);
		}
...
bool intersect(int x , int y){
	...
	for(int i = 0;i<len[x];i++){
		for(int j = 0;j<len[y];j++){
			if(intersectionSegments(p[x][i], p[x][(i + 1)%len[x]], p[y][j], p[y][(j+1)%len[y]], Inter)){
				return 1;
			}
		}
	}
...
int main() {
	...
	Emsawy();
	//cout<<IsIntersect(point(1,1), point(1,1), point(1,1), point(1,2)) << endl;
	while (cin >> n) {
		if(n == 0) break;
		while (getchar() != '\n');
		for(int i = 0;i<n;i++){
			...
			ss << s;
			int idx = 0;
			while(ss >> p[i][idx].x >> p[i][idx].y) idx++;
			len[i] = idx;
		}
		UnionFind uf(n);
		for(int i = 0;i<n;i++){
			...
			for(int j = 0;j<n;j++){
				for(int k = 0;k<len[j];k++){
					if(isInsidePolyBest(poly, p[j][k])){
						uf.union_set(i,j);
					}
				}
				if(intersect(i,j)){
