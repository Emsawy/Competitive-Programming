...
void compareFile() {
	...
	af.open("C:\\Users\\Emsawy\\Documents\\Visual Studio 2015\\Projects\\Emsawy\\Emsawy\\output.txt");
	bf.open("C:\\Users\\Emsawy\\Documents\\Visual Studio 2015\\Projects\\Emsawy\\Emsawy\\correct.txt");

	string a = "", b = "";
	
	while (getline(af, a) && getline(bf, b)) {
...
int main() {
	...
	while (cin >> a.x >> a.y >> b.x >> b.y >> n) {	
		if (a.x == 0 && a.y == 0 && b.x == 0 && b.y == 0 && n == 0) break;
		double x = hypot(abs(a.x - b.x), abs(a.y - b.y)) / 2;
		double angB = (2*PI / n);
		if (n % 2) {
			angB *= (ll) (n / 2);
			angB /= 2;
			x = dis(a, b) /  2 / sin(angB);
		}
		cout << fixed << setprecision(6) << n * x * x * cos(PI/n) * sin(PI / n) << endl;
 		//cout << fixed << setprecision(6) << n * x * x * sin(2 * PI / n) / 2 << endl;  0.5 * r * r * sin(ang)
	}
	return 0;
}
